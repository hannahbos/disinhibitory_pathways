...
class Setup(object):
    ...
    def get_circuit_params(self, circ, new_params):
        ...
        """
        new_vars = {}
        if circ.label == '3dcircuit':
            params, param_keys = pc.get_data_3dcircuit(new_params)
    ...
    def get_params_for_power_spectrum(self, circ):
        ...
        if circ.params['tf_mode'] == 'analytical':
            ...
            for key in ['tau_impulse_p', 'tau_impulse_n', 'delta_f_p',
                        'delta_f_n']:
    ...
    def get_altered_circuit_params(self, circ, label):
        ...
        new_vars = {}
        if label == '3dcircuit':
            params = pc.get_dependend_params_3dcircuit(circ.params)
        else:
            raise RuntimeError('Parameter file missing for label.')
