...
class Analytics(object):
    ...
    def get_mean(self, f):
        ...
        if 'Next_inh' in self.params:
            m_ext -= self.g*self.w*self.Next_inh*self.v_ext
        m = m0+m_ext
        return m
    ...
    def get_variance(self, f):
        ...
        # contribution from external sources
        var_ext = self.w*self.w*abs(self.Next)*self.v_ext
        if 'Next_inh' in self.params:
            var_ext += self.g*self.g*self.w*self.w*self.Next_inh*self.v_ext
        var = var0+var_ext
        return var

    def get_mean_Next(self, f, Next):
        '''Returns vector of mean inputs to populations depending on
        the firing rates f of the populations.
        Units as in Fourcoud & Brunel 2002, where current and potential
        have the same units, unit of output array: pA*Hz
        ...
        m0 = np.dot(self.I*self.W, f)
        # contribution from external sources
        m_ext = self.w*Next*self.v_ext
        if 'Next_inh' in self.params:
            m_ext -= self.g*self.w*self.Next_inh*self.v_ext
        m = m0+m_ext
        return m

    def get_variance_Next(self, f, Next):
        '''Returns vector of variances of inputs to populations depending
        on the firing rates f of the populations.
        Units as in Fourcoud & Brunel 2002, where current and potential
        have the same units, unit of output array: pA*Hz
        ...
        var0 = np.dot(self.I*self.W*self.W, f)
        # contribution from external sources
        var_ext = self.w*self.w*abs(Next)*self.v_ext
        if 'Next_inh' in self.params:
            var_ext += self.g*self.g*self.w*self.w*self.Next_inh*self.v_ext
        var = var0+var_ext
        return var
    ...
    def Next_integration(self, rates):
        ...
        if z[2] == 1:
            ...
            mu = self.get_mean_Next(rates, Next)
            sigma = np.sqrt(self.get_variance(rates))
            betas = 2*self.tauf/self.C*taum*np.array(map(
                transfer_function, taum*fac*mu, np.sqrt(taum)*fac*sigma))
        else:
    ...
    def create_H0_mu(self):
        ''' Returns matrix incorporating the transfer function and
        the instantaneous rate jumps at frequency zer0.
        '''
        taum = self.taum * 1e-3
        tauf = self.tauf * 1e-3
        taur = self.taur * 1e-3
        V0 = 0.0
        dV = self.Vth - self.V0
        trans_func = np.zeros(self.dimension, dtype=complex)
        for i in range(self.dimension):
            mu = self.mu[i]*taum/self.C
            sigma = np.sqrt(self.var[i]*taum)/self.C
            trans_func[i] = siegert.d_nu_d_mu_numerical(taum, tauf, taur, dV, V0, mu, sigma)
        # factor due to weight scaling of NEST in current equation
        # of 'iaf_psc_exp'-model
        fac = 2*self.tauf/self.C
        Delay_dist = self.create_delay_dist_matrix(0.0)
        if self.tf_mode == 'analytical':
            # find nearest omega, important when the transfer function is
            # read from file
            H = taum*fac*trans_func
            H = np.hstack([H for i in range(self.dimension)])
            H = np.transpose(H.reshape(self.dimension,self.dimension))
        else:
            print 'not available'
    ...
    def create_H0_WC(self):
        ...
        for i in range(self.dimension):
            mu = self.mu[i]*taum/self.C
            sigma = np.sqrt(self.var[i]*taum)/self.C
            trans_func[i] = siegert.d_nu_WC_d_mu_numerical(taum, tauf, taur,
                dV, V0, mu, sigma)
        # factor due to weight scaling of NEST in current equation
        # of 'iaf_psc_exp'-model
        fac = 2*self.tauf/self.C
    ...
    def create_H0_sigma(self):
        ...
        for i in range(self.dimension):
            mu = self.mu[i]*taum/self.C
            sigma = np.sqrt(self.var[i]*taum)/self.C
            trans_func[i] = siegert.d_nu_d_sigma_numerical(taum, tauf, taur, dV, V0, mu, sigma)
            trans_func[i] /= (2.*np.sqrt(self.var[i]))
        # factor due to weight scaling of NEST in current equation
        # of 'iaf_psc_exp'-model
        fac = 2*self.tauf/self.C
        Delay_dist = self.create_delay_dist_matrix(0.0)
    ...
    def create_MH0(self):
        ''' Returns effective connectivity matrix at zero frequency.'''
        H_mu = self.create_H0_mu()
        M = self.I*self.W
        H_sigma = self.create_H0_sigma()
        # Msigma = self.I*self.W*self.W
        # MH = H_mu*M+H_sigma*Msigma
        MH = (H_mu+H_sigma*self.W)*M
        # MH = H_mu*M
        return MH

    def create_MH0_WC(self):
        ''' Returns effective connectivity matrix at zero frequency.'''
        H_mu = self.create_H0_WC()
        M = self.I*self.W
        MH = H_mu*M
        return MH
    ...
    def spec_approx(self, omega):
        ...
        dominant eigenmod for all populations at frequency omega.
        '''
        MH_plus = self.create_MH(omega)
